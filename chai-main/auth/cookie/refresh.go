...
func (c *CookieAuth) Refresh(w http.ResponseWriter, r *http.Request) {
	...
	http.SetCookie(w, buildCookie("session_token", sessionToken, sessionExpiresAt, true))
	http.SetCookie(w, buildCookie("csrf_token", csrfToken, sessionExpiresAt, false))
	http.SetCookie(w, buildCookie("refresh_token", newRefreshToken, refreshExpiresAt, true))
